...
class _BuscaState extends State<Busca> {
  ...
  Future<void> _fetchPatrimonios() async {
    // ... (código _fetchPatrimonios idêntico à resposta anterior) ...
     setState(() { _isLoading = true; _errorMessage = ''; }); print("Executando _fetchPatrimonios (Busca)..."); const String url = "http://localhost/server/processa_bdCeet.php"; final Map<String, String> data = {'acao': 'listar'}; try { final response = await http.post( Uri.parse(url), headers: {'Content-Type': 'application/json; charset=UTF-8'}, body: json.encode(data), ).timeout(const Duration(seconds: 20)); print("Resposta Listar (${response.statusCode}): ${response.body.substring(0, min(response.body.length, 300))}..."); if (response.statusCode == 200) { final responseBody = json.decode(response.body); if (responseBody['status'] == 'success' && responseBody['produtos'] != null && responseBody['produtos'] is List) { print("Patrimônios recebidos com sucesso."); setState(() { _patrimonios = List<Map<String, dynamic>>.from(responseBody['produtos']) .where((p) => p['status'] != 'descartado') .map((p) => Map<String, dynamic>.from(p)) .toList(); _filteredPatrimonios = List.from(_patrimonios); _isLoading = false; print("Patrimônios atualizados no estado: ${_patrimonios.length} itens"); }); } else { print("Erro backend (listar): ${responseBody['message'] ?? 'Formato inválido.'}"); setState(() { _errorMessage = 'Erro ao buscar: ${responseBody['message'] ?? 'Formato inválido.'}'; _isLoading = false; }); } } else { print("Erro HTTP (listar): ${response.statusCode}"); setState(() { _errorMessage = 'Erro de comunicação (${response.statusCode}).'; _isLoading = false; }); } } catch (error, stackTrace) { print("Erro CATCH (listar): $error"); print("StackTrace Listar: $stackTrace"); setState(() { _errorMessage = 'Erro durante a requisição: $error'; _isLoading = false; }); }
  }
